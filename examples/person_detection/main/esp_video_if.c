...
static esp_err_t init_camera(v4l2_src_t *v4l2)
{
    ...
    ESP_LOGI(TAG, "Default: width=%" PRIu32 " height=%" PRIu32 " pixformat=%.4s", format.fmt.pix.width, format.fmt.pix.height, (char *) &(format.fmt.pix.pixelformat));
...
static video_fb_t *video_fb_get_cb(void *cb_ctx)
{
    ...
    for (int i = 0; i < BUFFER_COUNT; i++) {
        if (v4l2->fb_used[i] == false) {
            ...
            v4l2->fb.width = CAM_WIDTH;
            v4l2->fb.height = CAM_HEIGHT;
...
esp_err_t esp_video_if_start(void)
{
    ...
    uint32_t capture_fmt = V4L2_PIX_FMT_RGB565;
